...
public:
    ParallelKnn(int k) : neighbours_number(k) {}

    int parallel_predict_class(double* dataset[], const double* target, int dataset_size, int feature_size) {
        double* distances[3];
        int zeros_count = 0;
        int ones_count = 0;
        int prediction = -1;
        ...
        parallel_get_knn(dataset, target, distances, dataset_size, feature_size);
...
private:
    ...
    void parallel_get_knn(double* x[], const double* y, double* distances[3], int dataset_size, int feature_size) {
        ...
        for (int i = 0; i < dataset_size; i++) {
            ...
            auto task = taskflow.emplace([this, i, x, y, distances, feature_size]() {
...
int main() {
    ...
    else if (parallel_prediction == 1) {
        std::cout << "Parallel Predicted class: Prediabetes or Diabetes" << std::endl;
    }
    else {
        std::cout << "Parallel Prediction could not be made." << std::endl;
    }

    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    std::cout << "Serial Time difference = " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << "[µs]" << std::endl;
