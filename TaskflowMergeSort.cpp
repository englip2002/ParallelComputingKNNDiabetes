...
public:
    ...
    int predict_class(double* dataset[], const double* target, int dataset_size, int feature_size) {
        ...
        ////MergeSortParams tasks{ distances, dataset_size, 0, dataset_size - 1 };
        //    taskflow.emplace([=, &distances]() {
        //        parallel_merge_sort(distances, start, end - 1);
...
private:
    ...
    /*static void parallel_merge_sort(double** distances, int l, int h) {
        int low = l;
        int high = h;
    ...
    /*static void parallel_merge_sort(double** distances, int datasetSize, int l, int h) {
        int low = l;
        int high = h;
        ...
        if (low < high) {
            ...
            parallel_merge_sort(distances, datasetSize, low, middle);
            parallel_merge_sort(distances, datasetSize, middle + 1, high);
